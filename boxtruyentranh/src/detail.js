function execute(url) {
    ...
    var author = info.select(".flex-wrap")[0].select("span a").text()
    ...
    return Response.success({
        ...
        author : author,
        description : info.select(".wd-full")[0].select("p"),
        ongoing : ongoing.indexOf('Đang tiến hành')!=-1,
        detail : "Tác giả: " + author + "<br>" +
        "Lượt xem: " + info.select(".flex-wrap")[0].select("span")[1].text() + "<br>" +
        "Thể loại: " + String(info.select(".wd-full")[1].select(".mgen")).replace(/a> <a/g,'a>, <a').replace(/<[^>]+>/g,'').trim(),
    });
}